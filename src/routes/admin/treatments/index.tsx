...
function TreatmentsList() {
  ...
  const reorderMutation = useMutation({
    mutationFn: (orderedIds: string[]) => reorderTreatmentsFn({ data: { orderedIds } }),
    ...
    >
      ...
      {filtered.length > 1 && (
        <p className="mt-4 text-xs text-muted-foreground">
          ...
          treatments appear on the spa menu.
          {category === "All" && " Select a specific category above to keep the order tidy."}
        </p>
      )}
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="w-full min-w-[800px] border-collapse text-left text-sm">
                ...
                <tbody>
                  ...
                  >
                    {ordered.map((t) => (
                      <SortableItem key={t.id} id={t.id} as="tr" className="border-b border-border/60 bg-background">
